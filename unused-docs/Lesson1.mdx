...
## Welcome to [Web Development at Berkeley](https://none/)'s Introduction to Full-Stack Development DeCal!
...
<SimpleGrid columns={[1, null, 2]} spacing={[12, 16]}>
  {Board.map(({ name, image, role, linkedin, personal, bio, email }) => (
    <StaffMember
      name={name}
      image={image}
      role={role}
...
(hopefully) joining Salsa as an industry developer or designer, all in a
...
The course Board cares about one thing - making sure that you can learn and
...
Before continuing, please look over the [Course Policies](/Mission) to
...
3. Read through the [Course Policies](/Mission) for information on
